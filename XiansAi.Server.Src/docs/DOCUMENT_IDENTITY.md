...
The same identity governs the Admin Data API duplicate check on create and update, so the same
`type` + `key` may exist once per agent, activation and participant.
