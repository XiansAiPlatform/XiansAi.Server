...
| Document context | [`AdminApiTemporalDocumentDbContextAgentLifecycleTests`](../../../XiansAi.Server.Tests/IntegrationTests/AdminApi/AdminApiTemporalDocumentDbContextAgentLifecycleTests.cs) | `SaveAsync` / `GetByKeyAsync` from a Temporal **activity** and from **workflow** code (system `DocumentActivities` stub) |
...
| Webhook context | [`AdminApiTemporalWebhookContextAgentLifecycleTests`](../../../XiansAi.Server.Tests/IntegrationTests/AdminApi/AdminApiTemporalWebhookContextAgentLifecycleTests.cs) | `CreateAsync` / `ListAsync` / `DeleteAsync` from a Temporal **activity** and from **workflow** code (system `WebhookActivities` stub) |
...
dotnet test --filter "FullyQualifiedName~DocumentDbContextAgent_SaveGetByKey"
...
dotnet test --filter "FullyQualifiedName~WebhookContextAgent_CreateListDelete"
...
- Knowledge `GetAsync` / `ListAsync` from a Temporal activity and from workflow code (system `KnowledgeActivities` stub)
...
- Document DB `SaveAsync` / `GetByKeyAsync` from a Temporal activity and from workflow code (system `DocumentActivities` stub)
...
- Webhook `CreateAsync` / `ListAsync` / `DeleteAsync` from a Temporal activity and from workflow code (system `WebhookActivities` stub)
...
- Parent `[WorkflowRun]` `StartAsync` / `ExecuteAsync` / `SignalAsync` (Temporal child path; chat uses the client path)
