...
Lib-backed cycles (Echo chat/SSE, Knowledge overrides, Secret Vault strict scopes, Document DB, builtin Webhooks, file messaging, custom workflows, schedules, HITL tasks, cross-agent workflows, activations SDK) are documented in [Lib agent workflows](./lib-agent-workflows.md).
